...
const memberGroups = {
    ...
    leads: {
        ...
        members: [
            ...
            { name: "Nikunj Maheshwari", role: "Ops & Strategy", subgroup: "Leads", desc: "Managing operations and long-term strategic growth.", linkedin: "#", email: "nikunjnehu@gmail.com", github: "#", imageSrc: "images/team/nikunjmaheshwari_p.jpeg", thumbnailSrc: "images/team/nikunjmaheshwari_t.png" },
...
const Members = () => {
    ...
    const transitionContent = (callback) => {
        const targets = showGrid 
            ? contentRef.current 
            : [contentRef.current.querySelector('.ts-main-image'), contentRef.current.querySelector('.ts-text-content')];
        ...
        gsap.to(mainContent, { 
            ...
            onComplete: () => {
                ...
                if (window.innerWidth <= 768) {
                    ...
                    if (membersHeader) {
                        const y = membersHeader.getBoundingClientRect().top + window.scrollY - 80;
        ...
        <section id="members" className="section members-section">
            <div className="container">
                <div id="members-header-wrap" className="members-header">
                    <span className="section-tagline members-tagline reveal">OUR PEOPLE</span>
                    <h2 id="members-heading" className="section-title reveal" dangerouslySetInnerHTML={{ __html: group.title }}></h2>
                    <p id="members-subheading" className="section-subtitle-members">{group.subheading}</p>
                </div>
                ...
                <div className="members-layout">
                    ...
                    <div className="members-sidebar">
                        <div id="members-nav" className="members-nav-vertical">
                            {navItems.filter(item => item.id !== currentGroup).map(item => (
                                <button 
                                    key={item.id}
                    ...
                    <div className="members-display">
                        <div id="members-active-grid" className="members-slider-wrap reveal" ref={contentRef}>
                            {showGrid ? (
                                        ...
                                        {members.map((member, i) => (
                                            <div key={i} className="member-card reveal">
                                                <div className="member-card-img-wrap">
                                                    <img src={`/${member.imageSrc}`} alt={member.name} className="member-card-img" />
                                                </div>
                                                ...
                                                <div className="member-links">
                                                    <a href={member.linkedin} className="member-link" target="_blank" rel="noopener noreferrer"><i className="fa-brands fa-linkedin-in" style={{fontSize: '20px'}}></i></a>
                                                    <a href={`mailto:${member.email}`} className="member-link"><Mail size={20} /></a>
                                                    <a href={member.github} className="member-link" target="_blank" rel="noopener noreferrer"><i className="fa-brands fa-github" style={{fontSize: '20px'}}></i></a>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <div className="testimonial-slider-container">
                                    {currentGroup === 'leads' && (
                                        <button onClick={() => console.log('View All clicked')} className="view-all-btn-circular">
                                    ...
                                    <div className="ts-left-col">
                                        ...
                                        <div className="ts-thumbnails">
                                            {getThumbnails().map((member, i) => (
                                                ...
                                                >
                                                    <img src={`/${member.thumbnailSrc}`} alt={member.name} className="slider-thumb-img" />
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                                    <div className="ts-center-col">
                                        <img src={`/${activeMember.imageSrc}`} alt={activeMember.name} className="ts-main-image ts-main-image-anim" />
                                    </div>
                                    <div className="ts-right-col">
                                        <div className="ts-text-content ts-text-content-anim">
                                            ...
                                            <blockquote className="ts-quote">"{activeMember.desc}"</blockquote>
                                            <div className="member-links ts-contact-icons" style={{ marginTop: '2rem' }}>
                                                <a href={activeMember.linkedin} className="member-link" target="_blank" rel="noopener noreferrer"><i className="fa-brands fa-linkedin-in" style={{fontSize: '20px'}}></i></a>
                                                <a href={`mailto:${activeMember.email}`} className="member-link"><Mail size={20} /></a>
                                                <a href={activeMember.github} className="member-link" target="_blank" rel="noopener noreferrer"><i className="fa-brands fa-github" style={{fontSize: '20px'}}></i></a>
                                            </div>
                                        </div>
                                        <div className="ts-nav-buttons">
                                            <button onClick={handlePrev} className="ts-nav-btn"><ArrowLeft size={20} /></button>
                                            <button onClick={handleNext} className="ts-nav-btn ts-nav-btn-primary"><ArrowRight size={20} /></button>
                                        </div>
                                    </div>
                                </div>
                            )}
